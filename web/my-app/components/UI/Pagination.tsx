const Pagination = ({ page, setPage, quantity }: { page: number, setPage: (page: number) => void, quantity: number }) => {
    ...
    return (
        <section className='flex text-white text-lg my-6'>
            {page > 0 && <div onClick={pageHandler.bind(null, '-')} className='gradient rounded px-4 py-2 w-1/6 xl:w-1/12 text-center cursor-pointer mr-auto'>
                Previous
            </div>}
